...
/** Positive number from a query param, or "" (empty field) when absent/invalid. */
...
function CalculateurContent() {
  /**
   * Deep-link prefill: the analyser result and the À venir / Historique
   * tables hand off a price via /calculateur?adj=…&map=…&surface=… so the
   * breakdown renders on arrival. Read once in the useState initializers;
   * the fields own the state afterwards.
   */
  const searchParams = useSearchParams();

  const [adjudication, setAdjudication] = useState<number | "">(() =>
    numParam(searchParams.get("adj"))
  );
  const [miseAPrix, setMiseAPrix] = useState<number | "">(() =>
    numParam(searchParams.get("map"))
  );
  const [surface, setSurface] = useState<number | "">(() =>
    numParam(searchParams.get("surface"))
  );
  ...
  return (
    <main className="min-h-screen bg-[#0a0f1a] overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-10 space-y-6">
        ...
        {/* Inputs */}
        <section className="animate-fade-up bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl p-6">
        ...
        {!breakdown ? (
          ...
          <>
            {/* All-in summary card */}
            <section className="animate-fade-up relative overflow-hidden rounded-2xl border border-orange-500/20 bg-gradient-to-r from-orange-500/10 via-orange-500/5 to-transparent p-6 md:p-8">
...
/** useSearchParams (deep-link prefill) requires a Suspense boundary on a prerendered route. */
